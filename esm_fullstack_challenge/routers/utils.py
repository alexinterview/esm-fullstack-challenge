...
def get_route_list_function(table: str, table_model: BaseModel) -> Callable:
    ...
    def route_func_list_all(
        ...
        query_str, query_params = query_builder(
            table=table,
        ...
        count_query_str, count_params = query_builder(
            table=table,
            filter_by=cqp.filter_by,
            count_only=True
        )
        ...
        with db.get_connection() as conn:
            df = pd.read_sql_query(query_str, conn, params=query_params)
            cur = conn.cursor()
            cur.execute(count_query_str, count_params)
