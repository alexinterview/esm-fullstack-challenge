...
def autogen_models(db: str = 'data.db') -> Dict[str, BaseModel]:
    ...
    for table in tables:
        ...
        for k, v in df.dtypes.to_dict().items():
            base_type = type_map[str(v)]
            # Check if column has any null values - if so, make it Optional
            if df[k].isnull().any():
                types[k] = (Optional[base_type], Field(default=None))
            else:
                types[k] = (base_type, Field())
